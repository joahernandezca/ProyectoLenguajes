...
compilationUnit
	...
	| MODEL LF_BRACE logical_expression RG_BRACE ENDMODEL
	; 
...
proposition
	: (assign_statement)+ building_block SOLVE SEMICOLON	
	;

assign_statement
	: VARIBLE VAR_ID (inequation_operator numeric_literal)? SEMICOLON
	| PARAMETER ID ALLOCATE (option)? SEMICOLON 
	;
...
lineal_expression
	: numeric_literal MULT VAR_ID (lineal_operator lineal_expression)*
	| LF_PAREN lineal_expression RG_PAREN
	| VAR_ID
	| VAR_ID lineal_operator lineal_expression
	| numeric_literal aritmetic_operator LF_PAREN ( VAR_ID lineal_operator VAR_ID)+ RG_PAREN
	;
