...
class Functions():
    ...
    def simplex(self,objFun, const, typeM):
        var = len(objFun)
        numRes = len(const)
        cols = []
        resultCols = []
    ...
    def cohon(self,aObjFun, matrixConst, aTypeM):
        r = 5
        ...
        for i in xrange(numObjFun):
            solObjFun.append(self.simplex(aObjFun[i], matrixConst[i], aTypeM[i]))
    ...
    def run(self,Dictionary):
        self.cohon(Dictionary['objFun'], Dictionary['rest'], Dictionary['typeM'])
